...
def main():
    ...
    features_even = tracker_global_train['features'].data.cpu().numpy()[range(0,1),:,-1]
    features_odd = tracker_global_train['features'].data.cpu().numpy()[range(1,2),:,-1]
    ...
    for num in range(2):
        ...
        if num == 1:
            ...
            plt.savefig('odd_random_' + str(random) + '.png', dpi=600) 
            matplotlib2tikz.save('odd_random_' + str(random) + '.tex')  
            plt.close()
        ...
        if num == 0:
            ...
            width = 0.5
            p1 = plt.bar(np.arange(K), alpha_p.flatten(), width, color='blue', alpha = 0.5)
            p2 = plt.bar(np.arange(K), alpha_p.flatten(), width, color='orange', alpha = 0.5)
            p3 = plt.bar(np.arange(K), norm_singletons.flatten(), width, color='green', alpha = 0.5) # /1./np.sum(dst_obj.output_mass_singletons.flatten())
            ...
            plt.legend(['Softmax', 'Sparsemax', 'Ours'])
            plt.savefig('even_random_' + str(random) + '.png', dpi=600)  
            matplotlib2tikz.save('even_random_' + str(random) + '.tex')  
            plt.close()

''' Dempster-Shafer class for softmax parameters to be converted to masses. Deneoux paper. '''
import numpy as np
import itertools
...
class DST:
    ...
    def weights_from_linear_layer(self, weights, bias, features, mean):
        ...
        # use the Minimum Commitment Principle as in Denouex to identify parameters
        # Proposition 2 in Deneoux paper, mu: [J, 1], alpha: [J,K]
        # MAKE SURE BROADCASTING IS CORRECT HERE
        ...
        # alpha: [J, K]
        # print("CHECK", (self.beta * np.repeat(mean[:, None], self.K, axis=-1)).shape)
